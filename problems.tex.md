...
* **Bongjun: a bit confused with "the ouput size of the channel?". does it refer to the size of one of the feature maps? then Why batch size matter for computing the size of feature map?*

12. (1 point) Combining convolutional layers with fully connected layers can provide a boon in scenarios involving learning from images. Using your same `train` and `valid` DogSet subsets from questions 10 and 11, replace each of your first two hidden layers with a convolutional layer. When you call the PyTorch convolutional layer function, leave all of the arguments to their default settings except for kernel size and stride. Determine reasonable values of kernel size and stride for each layer and report what you chose. Train your neural network with the same training parameters as before (number of epochs, batch size, loss function, learning rate, etc). Report the results of your classifier.
...
14. (0.5 points) Using the model you trained in question 12, visualize the filters from your second convolutional layer. To do so, access the weights for that layer using `your_model.features[second_layer].weight.data`. You might need to clone this value to avoid issues. **HINT: Using [this function](https://pytorch.org/docs/stable/torchvision/utils.html?highlight=make_grid#torchvision.utils.make_grid) might be useful.**
